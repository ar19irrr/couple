...
def set_user_gender(chat_id, user_id, gender):
    ...
    data["profiles"][chat_id_str][str(user_id)] = profile
    save_data(data)
...
def update_monthly_score(chat_id, user_id, points=1):
    ...
    data["monthly_scores"][chat_id_str][user_id_str] = data["monthly_scores"][chat_id_str].get(user_id_str, 0) + points
    save_data(data)
...
def save_couple(chat_id, user1, user2):
    data = load_data()
    chat_id_str = str(chat_id)
    data["last_couple"][chat_id_str] = {"user1": user1, "user2": user2, "date": datetime.now().isoformat()}
    if "history" not in data:
        data["history"] = {}
    if chat_id_str not in data["history"]:
        data["history"][chat_id_str] = []
    data["history"][chat_id_str].append({"user1": user1, "user2": user2, "date": datetime.now().isoformat()})
    ...
    data["blocked"][chat_id_str].append({"user_id": user1["id"], "blocked_until": blocked_until})
    data["blocked"][chat_id_str].append({"user_id": user2["id"], "blocked_until": blocked_until})
    save_data(data)
...
def get_stats(chat_id):
    ...
    return {"total_members": len(members), "total_couples": total_couples, "unique_users": len(unique_users) - 1 if unique_users else 0, "last_couple": get_last_couple(chat_id)}
...
def get_user_couple_stats(chat_id, user_id):
    ...
    for partner_id, count in couple_count.items():
        ...
        result.append({"id": partner_id, "name": partner_info.get("name", "بدون نام"), "username": partner_info.get("username", "ندارد"), "count": count})
...
def get_user_total_couples(chat_id, user_id):
    ...
    for couple in history:
        ...
        u1 = couple.get("user1")
        u2 = couple.get("user2")
        if (u1 and isinstance(u1, dict) and u1.get("id") == user_id) or (u2 and isinstance(u2, dict) and u2.get("id") == user_id):
            total += 1
    return total
